<template>
  ...
  >
    <section
      ...
      :class="{'no-perspective': options.flat}"
    >
      <div
        ref="itemsWrapper"
        id="itemsWrapper"
...
export default {
  ...
  methods: {
    ...
    init_Mode() {
      ...
      const figures = itemsWrapper.getElementsByTagName('figure');
      for (let i = 0; i < figures.length; i += 1) {
        figures[i].style.width = `${this.carousel.panelWith}px`;//remove for test
        // figures[i].style.width = 'auto'; //add for test
      }
    },
...
.content-wrapper {
  ...
  font-family: appleUltralight,'SF UI Display Ultralight',  appleLight, 'SF UI Display Light';
}
/* 在此处设置渐隐动画 */
.hidden {
  opacity: 0;
