<template>
  <gree-view bg-color="#345b6e" class="page-voice">
    <gree-header theme="transparent">
      <gree-icon slot="overwrite-left" name="close" @click="goBack"></gree-icon>
      {{ $language('voice.voiceAssistant') }}
      <a slot="right" @click="prompt">{{ $language('voice.prompt') }}</a>
    </gree-header>
    <gree-page>
      <gree-popup v-model="isVoiceEnd" position="center" :has-mask="false">
        <p v-for="item in voiceList" :key="item.index">{{ item }}</p>
      </gree-popup>
      <div class="gree-messages-content" :class="{ hidden: isVoiceEnd }" v-chat-scroll>
        <div class="gree-messages">
          <div class="gree-message-wrapper" v-for="(item, index) in voiceTextList" :key="index">
            <!-- 每读一句语音，左右成对出现，所以右边必有一条 -->
            <div class="gree-message gree-message-sent" v-once>
            ...
            <div class="gree-message gree-message-received" v-once v-if="item.greekey.action && (!item.greekey.remind || item.greekey.action !== 'query_mode')">
              <div class="gree-message-content">
                <div class="gree-message-bubble">
            ...
            <div class="gree-message gree-message-received hint" v-once v-if="item.greekey.hint && !item.greekey.action">
              <div class="gree-message-content">
                <div class="gree-message-bubble">
    ...
    <gree-toolbar position="bottom" no-hairline>
      <gree-row>
        <gree-col>
          <div class="icon" @click="voiceBegin">
...
<script>
import { Header, Row, Col, Icon, Popup, ToolBar } from 'gree-ui';
import { mapState, mapMutations, mapActions } from 'vuex';
import { startSpeak, startVoice, stopSpeak } from '../../../static/lib/PluginInterface.promise';
...
export default {
  ...
  data() {
    return {
      isVoiceEnd: true,
      voiceList: this.$language('voice.prompt_power', 'voice.prompt_level', 'voice.prompt_add', 'voice.prompt_cycle'),
      voiceMsgTagList: {},
      voiceHintTagList: {},
      msg: ''
    };
    ...
    getVoiceMsg(greekey, index) {
      ...
      if (!this.voiceMsgTagList[index]) {
        ...
        if (action === 'control_on') {
          this.Pow ? (voiceCommander = this.setMsgOpt('已经是开机状态', '', '')) : (voiceCommander = this.setMsgOpt('已为您开机', '["Pow"]', '[1]'));
        } else if (action === 'control_off') {
          this.Pow ? (voiceCommander = this.setMsgOpt('已为您关机', '["Pow"]', '[0]')) : (voiceCommander = this.setMsgOpt('已经是关机状态', '', ''));
        } else {
          hint ? (this.msg = hint) : (this.msg = '您可以试试其他说法');
        }
        if (!this.Pow && action !== 'control_on' && action !== 'control_off') {
          this.msg = '关机状态下不能发送其它指令，请开机';
          voiceCommander = {};
        }
        if (Object.keys(voiceCommander).length !== 0) {
          this.msg = voiceCommander.msg;
