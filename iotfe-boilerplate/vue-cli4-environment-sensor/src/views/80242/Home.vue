    ...
    <gree-page no-navbar class="page-home">
      <div class="page-header" :style="{backgroundImage:'url(' + head_bg + ')'}">
        ...
        <gree-row class="data-list">
          <gree-col width="20" v-for="(item, index) in ColsList" :key="index">
            <p>{{ item.Value }}</p>
      ...
      <div class="page-main">
        <!-- 电池电量低 -->
        <gree-notice-bar icon v-if="[1, 2, 3].includes(BatteryWarn) && Pow">
          <gree-icon slot="left" name="warning" color="#f20c01"></gree-icon>
          <span>{{ $language("home.batteryWarn1") }}</span>
        </gree-notice-bar>
        <!-- 故障 -->
        <gree-notice-bar @click.native="$router.push({ name: 'Error' })" v-if="[1, 2, 3].includes(BatteryWarn) === false && Pow && SensorErr && !isMultiError">
          <gree-icon slot="left" name="warning" color="#f20c01"></gree-icon>
          <span>{{ errorListMixins[errorIndex].name }}</span>
        </gree-notice-bar>
        <gree-notice-bar
          @click.native="$router.push({ name: 'Error' })"
          v-else-if="[1, 2, 3].includes(BatteryWarn) === false && Pow && SensorErr && isMultiError"
        >
          <gree-icon slot="left" name="warning" color="#f20c01"></gree-icon>
          <gree-marquee>
            <gree-marquee-item v-for="(item, key) in errorMsg" :key="key">{{ item }}</gree-marquee-item>
          </gree-marquee>
        </gree-notice-bar>
        <!-- 警告 -->
        <gree-notice-bar icon v-else-if="[1, 2, 3].includes(BatteryWarn) === false && Pow && isWarningShow && !SensorErr">
          <gree-icon slot="left" name="warning" color="#f20c01"></gree-icon>
      ...
      <!-- 电源关闭 -->
      <gree-power-off v-model="showPowerOff" :text="$language('home.powerOff')" :style="{backgroundImage:'url(' + power_off_bg + ')'}"></gree-power-off>
    </gree-page>
    <!-- 底部弹出层按钮 -->
...
<script>
import { Header, Row, Col, NoticeBar, Icon, Marquee, MarqueeItem, PowerOff, ToolBar, Toast } from 'gree-ui';
import dayjs from 'dayjs';
import { mapState, mapMutations, mapActions } from 'vuex';
import homeConfig from '@/mixins/config/80242/home';
import errorConfig from '@/mixins/config/error';
import FunctionList from '@/components/80242/FunctionList';
import * as type from '@/store/types';
import { closePage, editDevice, changeBarColor, showToast } from '../../../../static/lib/PluginInterface.promise';
...
export default {
  ...
  watch: {
    ...
    InAirQuality: {
      handler(val) {
        const colors = ['#A5CD30', '#2998FF', '#F8CA31', '#FEA843'];
        if (val >= 0 && val <= 3) {
          this.statusBarColor = colors[val];
        } else {
          this.statusBarColor = colors[0];
        }
        changeBarColor(this.statusBarColor);
      },
      immediate: true
    },
    /**
    ...
    setFunction(val) {
      switch (val) {
        case 0:
          ...
          this.Pow ? this.setDataObject({ Pow: 0 }) : this.setDataObject({ Pow: 1 });
          break;
        case 1:
    ...
    getErrorIndex() {
      ...
      if (__SensorErr > 0) {
        ...
        if (parseInt(__SensorErr, 2) !== 2) {
          console.log(`故障发生，自动控制和灯带关闭：[二进制]${this.SensorErr}([十进制]${__SensorErr})`);
          console.log(' ');
