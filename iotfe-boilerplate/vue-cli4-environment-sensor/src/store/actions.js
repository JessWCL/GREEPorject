import { finishLoad, getInfo, getRoomDeviceSize, sendDataToDevice, updateStates } from '../../../static/lib/PluginInterface.promise'; // 主体接口
import {
  GET_ALL_STATES,
  GET_DEVICE_INFO,
  GET_TIMELIST,
...
/**
 * @description 获取房间内设备数量
 */
function getDeviceList({ state, commit }) {
  return getRoomDeviceSize(state.mac)
    .then(res => {
      const resultInfo = JSON.parse(res);
      commit(SET_DEVICE_LIST, resultInfo);
      return res;
    })
    .catch(err => {
      err;
...
function getStatusOfDev({ state, commit }) {
  ...
  return sendDataToDevice(state.mac, state.deviceInfo.fullstatueJson, false)
    .then(_res => {
      console.log('actions.js === getStatusOfDev', _res);
      const DataObject = {};
  ...
  async [SEND_CTRL]({ state, commit }, DataObject) {
    ...
    if (!state.isScene) {
      ...
      const _p = [
        ...
        state.dataObject.LigPow,
        state.dataObject.ScrTime,
        state.dataObject.AutoCtr,
        state.dataObject.TemSor,
        state.dataObject.HumSor,
        state.dataObject.HCHO,
        state.dataObject.PM2P5,
        state.dataObject.CO2,
        state.dataObject.InAirQuality,
        state.dataObject.SensorErr,
        state.dataObject.BatteryWarn
      ];
      // 成功之后更新主体状态
      r === 200 &&
