export const DEV_NAME = 'DEV_NAME'; // 设备名称
export const SET_DEVICE_INFO = 'SET_DEVICE_INFO'; // 设置deviceInfo
export const SET_MAC = 'SET_MAC'; // 设置mac
export const SET_DATA_OBJECT = 'SET_DATA_OBJECT'; // 设备的各种状态
export const SET_DEVICE_DATA = 'SET_DEVICE_DATA'; // 设备的各种状态
export const SET_FUNCTYPE = 'SET_FUNCTYPE'; // 是否场景模式
export const SET_LOCK = 'SET_LOCK';
export const EDIT_ERCODE = 'EDIT_ERCODE'; // 设置故障代码
export const EDIT_ENABLE = 'EDIT_ENABLE'; // 是否是消息界面
export const SET_SUBSCRIBE_TIMER = 'SUBSCRIBE_TIMER'; // 设置的预约时间（小时）

// 根据业务逻辑修改
export const GET_DEVICE_INFO = 'GET_DEVICE_INFO'; // 获取设备信息
export const GET_ALL_STATES = 'GET_ALL_STATES'; // 获取全部状态
export const SEND_CTRL = 'SEND_CTRL'; // 发送控制指令
export const SET_INIT_STATUS = 'SET_INIT_STATUS'; // 设置初始化状态
export const SET_LOADING = 'SET_LOADING'; // 设置loading的显示
export const SET_TIMER_VALUE = 'SET_TIMER_VALUE'; // 设置定时（预约）的值
export const SET_ENVIRONMENTAL_HUMIDITY = 'SET_ENVIRONMENTAL_HUMIDITY';// 设置环境湿度
export const SET_FUNCTYPE_DATA = 'SET_FUNCTYPE_DATA'; // 设置故障
