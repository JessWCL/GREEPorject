import { finishLoad, getInfo, sendDataToDevice, updateStates } from '../../../static/lib/PluginInterface.promise'; // 主体接口
import { GET_ALL_STATES, GET_DEVICE_INFO, SEND_CTRL, SET_DATA_OBJECT, SET_DEVICE_INFO, SET_LOCK } from './types';
...
export default {
  ...
  async [SEND_CTRL]({ state, commit }, DataObject) {
    ...
    if (!state.isScene) {
      ...
      const { r } = result;
      const _p = [state.dataObject.WorkState, state.dataObject.AlarmCancel, state.dataObject.AlarmTmrEnable, state.dataObject.AlarmTime];
      // 成功之后更新主体状态
      r === 200 &&
