 ...
 * @LastEditTime: 2020-06-17 16:19:44
  ...
  methods: {
    ...
    async saveTimer() {
      if (this.routerName === 'modify') {
        const item = this.groups[this.modifyIndex];
        const Data = {
          group_id: item.id,
          message: {
    ...
    fillData() {
      if (this.routerName === 'modify') {
        ...
        this.setType = +timer.functions[0].value;
        this.timeDefalut = [this.hour, this.min];
        this.repeat = parseInt(timer.loops, 2); // repeat这里是十进制的所以要转化
        this.selectDay = this.toStringBinaryList(this.repeat);
