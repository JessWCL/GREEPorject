 ...
 */
function SetDevCustomName({ state, commit }) {
  return SetDevCustomData(
    ...
    .then(res => {
      ...
      if (resMsg.r === 10700) {
        ...
        GetDevCustomName({ state, commit }); // 还原以前数据
 ...
 */
function GetDevCustomName({ state, commit }) {
...
function getStatusOfDev({ state, commit }) {
  ...
  const dataBox = state.DataBoxObject;
          ...
          if (cols[i] === 'GasQ' && res[i] !== 0) {
            ...
            const getBoxData = async () => {
              for (let z = 0; z < chargeOnesCountInNum2(res[i]); z += 1) {
                const sJson = `{"t":"status","mac":"${state.g_mac}","GasN":${z},
                "cols":["TemSor","HumSor","PM2P5","CO2","GasMas","GasLED",
                "GasAvail","GasMod","AirQu","SorErr","CommErr","PM2P5Sta"]}`;
...
export default {
  ...
  async [GET_ALL_STATES]({ state, commit }) {
    if (!state.isScene) {
      ...
      // await GetDevCustomName({ state, commit }).then(res => res);
      if (_timer === 0) {
        _timer = setInterval(() => {
  ...
  async [GET_BOX_NAME]({ state, commit }) {
    if (!state.isScene) {
      await GetDevCustomName({ state, commit }).then(res => res);
    }
  },
  ...
  async [SET_BOX_NAME]({ state, commit }) {
    if (!state.isScene) {
      await SetDevCustomName({ state, commit }).then(res => res);
    }
  },
  /**
   * @description 发送控制指令
