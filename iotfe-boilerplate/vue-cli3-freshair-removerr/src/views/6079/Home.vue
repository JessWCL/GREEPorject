  ...
  <gree-view bgColor='#AACD52'>
    ...
    >
      ...
      >
        ...
        <gree-row>
          <div class="page-line" :class="{hidden: ChargeOnesCountInNum2() !== 2}" v-show="!isScene"></div>
          <gree-col
            ...
            <div class="air-quality">
              <span class="air-level">{{ Boolean(airQ[item.AirQu])? airQ[item.AirQu]: '--'}}</span>
            </div>
            <div class="room-name"> 
        ...
        <gree-block class="notice-err" v-show="!isScene">
          <!-- 故障 -->
      ...
      <div class="page-main" v-show="!isScene">
        ...
        >
          ...
          <span v-if="PctRe">{{ $language("home.PctRe") }}<a @click="buyAddress()" class="buy">{{ $language("home.buy") }}</a></span>
      ...
      <gree-block class="last-extend">
        ...
        <div class="fan-text">
          ...
          <img @click="showPopUp3('center')" v-if="this.FanMod === 3 && !isScene"
            class="imgtips" 
...
import homeConfig from '@/mixins/config/6079/home';
import errorConfig from '@/mixins/config/error';
import watchMod from '@/mixins/utils/watch-mod';
import FunctionList from '@/components/6079/FunctionList';
...
import CarouselFan from '../../components/6079/carousel/CarouselFan';
import CarouselMove from '../../components/6079/carousel/CarouselMove';
export default {
  components: {
    FunctionList,
    CarouselFan,
  ...
  computed: {
    ...
    head_bg() {
      let bg = require('@/assets/img/bg_box_n.png');
      return bg;
    }
  },
  watch: {
    ...
    ModFanText(newValue, oldValue) {
      ...
      this.setCarouselIdFan('carousel-fanmode', index);
    ...
    ModMoveText(newValue, oldValue) {
      ...
      this.setCarouselIdMove('carousel-movemode', index);
      console.log(newValue, oldValue, index);
    }
  },
  created() {
  ...
  mounted() {
    this.getErrorMsg();
    const fanval = this.WdSpd;
    const fanindex = this.allState.FanMode.range.indexOf(fanval);
    this.setCarouselIdFan('carousel-fanmode', fanindex);
    const moveval = this.FanMod;
    const moveindex = this.allState.MoveMode.range.indexOf(moveval);
    this.setCarouselIdMove('carousel-movemode', moveindex);
  },
  methods: {
    ...mapMutations({
