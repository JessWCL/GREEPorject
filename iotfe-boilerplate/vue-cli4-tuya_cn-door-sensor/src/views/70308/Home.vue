<template>
  <gree-view :bg-color="statusBarColor">
    <gree-page no-navbar class="page-home">
            ...
            <gree-scroll-view ref="scrollView" :scrolling-x="false" manual-init :bouncing="false" @end-reached="onEndReached">
              <gree-list>
                <gree-list-item :title="item.event_time" v-for="(item, index) in lists" :key="index">
                  <div slot="after" v-if="item.value === 'true'">门窗开启</div>
                  <div slot="after" v-else>门窗关闭</div>
                </gree-list-item>
              </gree-list>
      ...
      <div v-show="false">{{ doorcontact_state }} - {{ battery_percentage }} - {{ ColorChange }}</div>
    </gree-page>
  </gree-view>
</template>
...
import { closePage, tuyaDeviceMore, changeBarColor, tuyaGetDevLogs } from '../../../../static/lib/PluginInterface.promise';
  ...
  mounted() {
    ...
    tuyaGetDevLogs(this.mac, startTime, endTime, logType, logSize)
      .then(res => {
        ...
        // console.log('tuyaGetDevLogs', __res);
        if (__res.code === 200) {
          this.lists = __res.message.result.logs.filter(item => item.code === 'doorcontact_state');
