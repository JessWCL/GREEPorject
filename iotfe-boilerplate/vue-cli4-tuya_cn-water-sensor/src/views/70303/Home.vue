<template>
  <gree-view :bg-color="statusBarColor">
    <gree-page no-navbar class="page-home">
        ...
        <div class="battery-remind" v-if="battery_percentage >= 0">
          ...
          <h3>
            ...
            <div class="status" :class="{ low: battery_percentage <= lowBatteryLimit }">
              <div class="acid" :style="{ height: battery_percentage + '%' }" />
            </div>
          </h3>
        </div>
        <gree-activity-indicator class="battery-remind-loading" type="carousel" :size="10" color="white" v-else />
        <!-- 警告 -->
        <gree-notice-bar ref="custom_notice_bar" v-if="watersensor_state === '1'">
          <img src="@/assets/img/warning.png" slot="left" />
          <span>{{ $language('home.warning') }}</span>
          <a href="javascript:;" @click="close">点击关闭</a>
        </gree-notice-bar>
      </div>
      <div class="page-main">
      ...
      <div v-show="false">{{ watersensor_state }} - {{ battery_percentage }} - {{ temper_alarm }} - {{ ColorChange }}</div>
    </gree-page>
  </gree-view>
</template>

<script>
import { Header, List, Item, Block, Icon, NoticeBar, ActivityIndicator, ScrollView, ScrollViewMore, Popup, Toast } from 'gree-ui';
...
export default {
  ...
  computed: {
    ...
    temper_alarm() {
      return this.dataObject.properties[2].value;
    },
    /**
  ...
  mounted() {
    ...
    tuyaGetDevLogs(this.mac, startTime, endTime, logType, logSize)
      .then(res => {
        ...
        if (__res.code === 200) {
          this.lists = __res.message.result.logs.filter(item => item.code === 'watersensor_state');
