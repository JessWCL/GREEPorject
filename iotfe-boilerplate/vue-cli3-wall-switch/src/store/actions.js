...
function getStatusOfDev({ state, commit }) {
  ...
  return sendDataToDevice(state.mac, state.deviceInfo.fullstatueJson, false)
    .then(_res => {
      ...
      let res = '';
      // TODO:这里是重点
      if (process.env.VUE_APP_MID === '80802') { // mesh 80802
        res = JSON.parse(_res).dat;
      } else { // wifi
        res = JSON.parse(_res);
      }
      ...
      console.log(`%c ===== ${process.env.VUE_APP_MID} 轮询 =====', 'color: red;`);
      console.log(DataObject);
      if (!state.isScene) {
        // 非场景时提交数据
        if (ignoreSetData) {
          console.log('%c 是否忽略这一包', 'color: red;', ignoreSetData);
          return false;
        }
...
export default {
  ...
  async [SEND_CTRL]({ state, commit }, DataObject) {
    ...
    const mac = state.mac;
    const t = 'cmd';
    let json = JSON.stringify({ mac, t, opt, p });
    if (process.env.VUE_APP_MID === '80802') { // mesh 80802
      json = JSON.stringify({ t, opt, p });
    }
