 ...
 * @LastEditTime: 2020-10-14 17:02:48
 * @Description: 新电量统计页面 V2.2  2代表黎工1.0模板电量页面，小版本1代表增加了坐标轴等较大改动
 -->
<template>
  <gree-view class="page-electric" bgcolor="##FFFFFE">
    <gree-header
...
import chartsConfig from '../mixins/config/charts.js';
...
export default {
  ...
  methods: {
    ...
    async btnIndex(index) {
      ...
      switch (index) {
        case 0:
          this.chartData.dayChart ? this.dayChartSet(this.tooltip) : await this.getDayData();
          this.isNoData = !!this.chartData.dayChart[2].length;
          console.log('------', this.chartData.dayChart[2]);
          break;
        case 1:
          this.otherOpt.tooltip.formatter = '当日用电量：{c} kW·h';
          this.chartData.weekChart ? this.weekChartSet(this.tooltip, this.dataZoom) : await this.getWeekData();  
        ...
        case 3:
          ...
          this.chartData.yearChart ? this.yearChartSet(this.tooltip, this.dataZoom) : await this.getYearData();
          this.isNoYearData = !!this.chartData.yearChart[2].length;
          break;
        default:
          this.chartData.dayChart ? this.dayChartSet(this.tooltip) : await this.getDayData();
