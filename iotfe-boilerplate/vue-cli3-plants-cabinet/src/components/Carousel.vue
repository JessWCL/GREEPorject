<template>
  <div
    ...
    :style="{width:options.width,marginLeft:options.marginLeft,marginRight:options.marginRight,}"
  ...
  methods: {
    ...
    _touchstart(event) {
      if (!this.options.controlAble) {
        this.$emit('uncontrolable');
        return;
      }
      //  状态监控：
      if (event.touches.item(0).identifier === event.targetTouches.item(0).identifier) {
        event.preventDefault(); //  阻止浏览器默认事件，重要
        this.$emit('hidePicture');
        this.isEdit = true;
        this.isEditTime = event.timeStamp || Date.now();
        const touch = event.targetTouches[0];
        // const touch = event.touches.length === 1 ? event.targetTouches[0] : event.touches[0];
    ...
    _touchmove(event) {
      ...
      if (event.targetTouches.length !== 0) {
        ...
        let ChangeRotate = valpresect * 0.5; // 调整touchmove的ChangeRotate和touchend的dist来控制滑轮灵敏度
        ChangeRotate = Number(ChangeRotate.toFixed(4));
        ...
        if (this.selectIdCache !== this.selectId) {
          this.selectIdCache = this.selectId;
          //  向父组件传出事件：
          this.$emit('currentItemChange', this.computedData[this.selectId].name);
        }
      }
    },
    _touchend(event) {
      ...
      const dist = (v * duration) / 10; // 最终移动多少 // 调整touchmove的ChangeRotate和touchend的dist来控制滑轮灵敏度
      ...
      this.$emit('currentItemChange', null); // 清除移动的模式名字
      this.clear();
    },
    clear() {
