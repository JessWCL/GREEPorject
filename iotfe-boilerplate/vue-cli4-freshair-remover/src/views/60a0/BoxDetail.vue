        ...
        <gree-list>
          <ul>
            <div class="item-content">
              <div class="item-inner">
                <div class="item-title extend-title">名字</div>
                <div v-if="textflag" class="item-after extend-title" @click="edit()">{{ roomName }}</div>
                <input class="edit-text" type="text" v-else @change="input()" ref="inputValue" />
              </div>
            </div>
          </ul>
          <!-- <gree-list-item title="开启空气盒子">
            ...
            <gree-switch
              slot="after"
              v-model="GasAvail"
              @change="handlerSwitch('GasAvail', GasAvail, $event)"
              active-color="#5eb79d "
          ...
          <gree-list-item title="设为主空气盒子">
            ...
            <gree-switch
              slot="after"
              v-model="GasMas"
              @change="handlerSwitch('GasMas', GasMas, $event)"
              active-color="#5eb79d "
              inactive-color="#e6e6e6"
              :disabled="!GasAvail"
            ></gree-switch>
          ...
          <gree-list-item title="呼吸灯">
            ...
            <gree-switch
              slot="after"
              v-model="GasMod"
              @change="handlerSwitch('GasMod', GasMod, $event)"
              active-color="#5eb79d "
              inactive-color="#e6e6e6"
              :disabled="!GasAvail"
            ></gree-switch>
          ...
          <gree-list-item title="PM2.5检测">
            ...
            <gree-switch
              slot="after"
              v-model="PM2P5Sta"
              @change="handlerSwitch('PM2P5Sta', PM2P5Sta, $event)"
              active-color="#5eb79d "
              inactive-color="#e6e6e6"
              :disabled="!GasAvail"
            ></gree-switch>
...
import { Dialog, Header, Item, List, Switch } from 'gree-ui';
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      // GasAvail: state => state.DataBoxData[state.GasN].GasAvail,
      // GasLED: state => state.DataBoxData[state.GasN].GasLED,
      // GasMod: state => !state.DataBoxData[state.GasN].GasMod,
      // GasMas: state => state.DataBoxData[state.GasN].GasMas,
      // PM2P5Sta: state => state.DataBoxData[state.GasN].PM2P5Sta,
      // SorErr: state => state.DataBoxData[state.GasN].SorErr,
      // CommErr: state => state.DataBoxData[state.GasN].CommErr,
  ...
  watch: {
    ...
    DataBoxData: {
      handler(newval) {
        this.GasAvail = newval[this.GasN].GasAvail ? true : false;
        this.GasLED = newval[this.GasN].GasLED ? true : false;
        this.GasMod = newval[this.GasN].GasMod ? false : true;
        this.GasMas = newval[this.GasN].GasMas ? true : false;
        this.PM2P5Sta = newval[this.GasN].PM2P5Sta ? true : false;
        this.SorErr = newval[this.GasN].SorErr ? true : false;
        this.CommErr = newval[this.GasN].CommErr ? true : false;
      },
      immediate: true
    }
  },
  ...
  methods: {
    ...
    input() {
      this.roomName =
        this.$refs.inputValue.value.length > 10
          ? this.$refs.inputValue.value.slice(0, 10) + '...'
          : this.$refs.inputValue.value;
      //this.$store.commit(`${type.SET_HOME`${this.GasN}_OBJECT}`,this.roomName)
      // 最多两个盒子暂时写了两个(接手大哥可以优化下，本人没时间写了)
      if (this.GasN === 0) {
    ...
    handlerSwitch(name, active) {
      ...
      switch (name) {
        ...
        case 'GasMod':
          ...
          var c = LEDArray.reverse(); // 倒序数组值
          for (let z = 0; z < c.length; z++) {
            ledVal = ledVal + c[z];
          }
          var ledResult = parseInt(Number(ledVal), 2);
