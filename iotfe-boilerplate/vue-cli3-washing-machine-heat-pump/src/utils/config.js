 ...
 * @LastEditTime: 2020-05-30 19:44:59
 */
//  参数中的中文转数字特殊处理
...
const dryValueList = ['不烘干', '标准', '超干', '30分', '60分', '90分', '120分', '150分', '180分', '210分', '240分', '熨烫', '经济', '快速'];
...
export const valueChangeZn = (value, protocol) => {
  ...
  switch (protocol) {
    case 'washTemp': // 温度
      realValue = realValue || (process.env.VUE_APP_MID === '28a01' ? '不加热' : '--'); // 只有为0需要特殊处理, 热泵显示不加热
      break;
    case 'speed':
      realValue = value || (process.env.VUE_APP_MID === '28a01' ? '不脱水' : '--');
      break;    
    case 'dry': // 烘干
