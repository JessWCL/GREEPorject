  ...
  <div class="body-main">
    ...
    <gree-block>
      ...
      <gree-row v-if="order">
        <gree-col width="10"/>
        <gree-col width="35">
          <gree-button @click="handleCancel" round :class="{opacity: specialLock}" style="width: 100%">{{
            $language('timer.cancel')
          }}</gree-button>
        </gree-col>
        <gree-col width="35">
          <gree-button @click="handlePause" round :class="{opacity: specialLock, textBlue: !orderLaunch}" style="width: 100%" >{{
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      orderTimeDay: state => state.dataObject.orderTimeDay,
      orderTimeHour(state) {
  ...
  methods: {
    ...
    handlePause() {
      ...
      const commitObj = {};
      if (this.specialLock) commitObj.specialLock = 0;
      this.setDataObject(Object.assign(commitObj, { orderLaunch }));
    ...
    handleCancelConfirm() {
      const commitObj = {};
      if (this.specialLock) commitObj.specialLock = 0;
      this.setDataObject(Object.assign(commitObj, { order: 0 }));
