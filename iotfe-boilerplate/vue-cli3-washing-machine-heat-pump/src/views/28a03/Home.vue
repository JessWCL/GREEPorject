  ...
  <gree-view bg-color="#4DB6CF">
    ...
    <gree-page class="page-home" no-navbar>
      ...
      >
        ...
        <!-- 时间 -->
        <div class="mode-time2" v-if="devState && devState !== 3 && devState !== 5">
          {{ timeLeft | timeConversion }}
        </div>
        <div class="mode-time2" v-else>  <!-- 待机状态下显示这个 -->
          {{ timeAll | timeConversion }}
        </div>

        <div class="mode-name2" @click="onTest('stepTwo')">
        ...
        <div class="data-list-div-div">
          <div class="data-list-div">
            ...
            <div :class="washList[modeIndexList[washMode]].configList.length > 4 && howPage === 1 ? 'data-list-arrow' : 'data-list'">
              ...
              <div
                ...
                :class="{'data-block' : washList[modeIndexList[washMode]].configList.length > 4}"
              >
                <div
...
export default {
  ...
  methods: {
    ...
    ...mapActions({ 
      sendCtrl: 'SEND_CTRL',
      getPowOnState: 'GET_POW_ON_STATES'
    }),
    /**
     * @function goBack
     * @description 返回键
    ...
    pickerOnChange(event, inst) {
      ...
      if (_this.clickProtocol === 'washTemp') {
        ...
        const beforeWashTime = configList.defalutWashTimeList[beforeValue];
      ...
      if (_this.clickProtocol === 'dry') {
        ...
        if (_this.nightWash) {
          addTime -= list.auxiliaryTimeList.nightWash;
          sendobj.nightWash = 0;
          if (!_this.speed) {
    ...
    handleStartBtn() {
      ...
      // 高级功能的选择取值(先取键名再取高级功能中的点击)
      const advanceKey = [];
      const advanceObj = {};
      this.advancedList.forEach(v => {
        advanceKey.push(v.protocol);
      });
      this.advancedList.forEach((element, index) => {
        element.selected ? (advanceObj[advanceKey[index]] = 1) : '';
      });
      Object.assign(sendData, advanceObj);

      //  烘干状态取值
      const drying = {};
      this.dryFuncList.forEach(element => {
        if (element.selected) {
          drying.dry = element.protocolNumber;
        }
      });
      Object.assign(sendData, drying);

      // 参数配置的选择取值
      const list = this.washList[0].washMode[this.HDwashModeIndex].configList;
      const configObj = {};
      list.forEach(element => {
        configObj[element.protocol] = changeZnVlue(element.value);
      });
      Object.assign(sendData, configObj);
    ...
    toggle(index) {
      ...
      if (protocolValue === 'speed' && this.dry) {
        ...
        case 0:
          if (!this.isCloseing) {
            ...
            if (!obj.Pow) { // 关机5秒不可开机
              ...
              // 关机再开机恢复默认值
              const {configList, auxiliaryFunction, defaultTime } = this.washList[this.washModeIndex];
              configList.forEach(element => {
                commitObj[element.protocol] = changeZnValue(element.value, element.protocol);
              });

              const keys = Object.keys(auxiliaryFunction);
              keys.forEach(protocol => {
                commitObj[protocol] = Number(auxiliaryFunction[protocol] === 'U');
              });
              commitObj.timeAll = defaultTime;
