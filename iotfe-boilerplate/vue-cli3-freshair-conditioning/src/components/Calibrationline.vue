<template>
  <div class="box-bg-img">
    <div class="typeVal" v-for="(item,index) in IndicatorList">
       ...
       <span class="typeVal_text">当前值: {{item.val + item.unit}}</span>
       <div class="scale" :style="{'width': `${scalewidth}rem`}">
         <div class="tips-text" :style="{'width': `${tipsWidth}rem`,'height': `${tipsHeight}rem`,'left': `${getClientLeft(item.type)}rem`}">{{item.val}}</div>
          <span class="rules" v-for="(items,index) in item.number" :style="{'width': `${weight[item.type][index]*scalewidth}rem`}"/>  
       </div>
...
export default {
  data() {
    return {
       ...
       */
      weight: {
        'eCO₂': [600 / 2000, 400 / 2000, 600 / 2000, 400 / 2000],
  ...
  methods: {
    getClientLeft(type) {
      ...
      switch (type) {
        case 'eCO₂':
          val = this.CO2 > 2000 ? 2000 : this.CO2;
          max = 2000;
          this.$set(this.IndicatorList[0], 'val', this.CO2);
          break;
        case 'PM2.5':
          val = this.PM2P5 > 150 ? 150 : this.PM2P5;
          max = 150;
          this.$set(this.IndicatorList[1], 'val', this.PM2P5);
          break;
        case 'HCHO(甲醛)':
      ...
      return (val / max) * this.scalewidth - this.tipsWidth / 2;
    }
  }
};
</script>
