 ...
 * @version: 1.1
 * @Author: ZM_lee└(^o^)┘
 * @Date: 2019-09-06 11:38:20
 * @LastEditors: ZM_lee└(^o^)┘
 * @LastEditTime: 2019-11-01 12:41:30
 -->
<template>
  <div
    ref="wrapper"
        ...
        >
          ...
          >
            <div
              ...
              :style="{height:options.height,
                       fontSize:fontSizeChange[item.id],
            ...
            >
              {{ item.content }}<span
                ...
                :style="{fontSize:fontSize0_5, color:'#000000'}">.5
              </span>
            </div>
            <img
              v-if="options.isShow && !options.showNumOrImg"
...
export default {
  ...
  methods: {
    ...
    _touchend(event) {
      ...
      //  向父组件传出事件：
      this.incomingId !== this.selectId ? this.$emit('currentChange', this.selectId) : '';
      this.clear();
    },
    clear() {
    ...
    _setId(id) {
      ...
      if (id == this.selectId || this.isEdit || !isInt || id < 0 || id > this.propData.length - 1) return;
      //  此处单独设置取消了此操作的动画，需要切换动画时屏蔽此段代码
      const itemsWrapper = this.$el.querySelector('#itemsWrapper');
      // const figures = itemsWrapper.getElementsByTagName('figure');
      itemsWrapper.style.transition = 'transform 0s';
