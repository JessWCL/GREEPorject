    ...
    <gree-page class="page-home" no-navbar>
            ...
            <div :class="washList[modeIndexList[washMode]].configList.length > 4 && howPage === 1 ? 'data-list-arrow' : 'data-list'">
              ...
              >
                ...
                <p class="protocol-value" 
                   :class="{'visibility-hidden' : isActive && index === clickConfigIndex}" 
                   @click="showPicker(index)" ref="trigger">
                ...
                <p class="protocol-name" @click="showPicker(index)" :class="{'mbsc-bg': isActive && index === clickConfigIndex }">{{ washConfigDescribe[0][item.protocol] }}</p>
              </div>
      ...
      >
        <gree-row>
          ...
          >
            <div class="icon">
              <img :src="require('@/assets/img/Icon/functionBtn/' + item.IconName + '.png')"/>
            </div>
...
import { changeZnValue, valueChangeZn, hiddenUnit, stainsList} from '@/utils/config';
import { getDisableObj } from '@/utils/index';

import ModeList from '@/components/28a06/ModeList';
import middleFunclist from '@/components/28a06/middleFunclist';
import middleAppointment from '@/components/28a06/middleAppointment';
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      devState: state => state.parseObj.drum2Params.devState, // 当前整机状态
      washMode: state => state.parseObj.drum2Params.washMode,
      timeAll: state => state.parseObj.drum2Params.timeAll, // 总设定的时间
      timeLeft: state => state.parseObj.drum2Params.timeLeft, // 洗涤剩余时间
      runStage: state => state.parseObj.drum2Params.runStage, // 当前运行阶段
      order: state => state.parseObj.drum2Params.order, // 预约
      orderLaunch: state => state.parseObj.drum2Params.orderLaunch, // 预约
      dataObject: state => state.parseObj.drum2Params,
      washTemp: state => state.parseObj.drum2Params.washTemp,
      setWashTime: state => state.parseObj.drum2Params.setWashTime,
      potch: state => state.parseObj.drum2Params.potch,
      speed: state => state.parseObj.drum2Params.speed,
      stains: state => state.parseObj.drum2Params.stains,
  ...
  methods: {
    ...
    setPickerList() {
      ...
      configList.forEach(element => {
        const list = element.list.length === 3 ? element.list.concat(element.list) : element.list
    ...
    showPicker(index) {
      ...
      const isStains = this.stains && (protocol === 'washTemp' || protocol === 'setWashTime');
    ...
    handleStartBtn() {
      ...
      this.setMiddleObj(Object.assign(commitObj, sendObj));
