  ...
  <gree-view bg-color="#4DB6CF">
    ...
    <gree-page class="page-home" no-navbar>
      ...
      >
        ...
        <gree-notice-bar
          icon="bell"
          id="powoff-notice-bar"
          v-show="!Pow && warningStatus"
        >{{ warnMsg }}
        ...
        <div class="data-list-div-div">
          <div class="data-list-div">
            ...
            <div :class="washList[washModeIndex].configList.length > 4 && howPage === 1 ? 'data-list-arrow' : 'data-list'">
              ...
              >
                <div
                  :id="`TimerSelector_${item.protocol}`"
                  class="timer-elector"
                  :class="{'visibility-hidden' : !isActive || index !== clickConfigIndex}"
                  :style="{bottom: errStatus ? '57px': ''}"
                />

                <p class="protocol-value" 
                   :class="{'visibility-hidden' : isActive && index === clickConfigIndex, 'opacity5': isOpacity(item) }" 
                   @click="toggle(index)" ref="trigger">
                  {{ valueChangeZn(dataObject[item.protocol], item.protocol) }} 
                  <span class="protocol-unit" :class="{'hidden': hiddenUnit(dataObject[item.protocol], item.protocol)}"> 
                    {{ washUnit[item.protocol] }}
                  </span>
                </p>
                <p class="protocol-name" @click="toggle(index)" :class="{'mbsc-bg': isActive && index === clickConfigIndex , 'opacity5': isOpacity(item)}">{{ washConfigDescribe[0][item.protocol] }}</p>
              </div>
      ...
      <div class="start-or-timer">
        <!-- 提醒提示 -->
        <gree-notice-bar
          v-show="warningStatus && !appointmentStatus && Pow"
          icon="bell"
...
import { changeZnValue, valueChangeZn, hiddenUnit, dryValueObj} from '../../utils/config';
...
export default {
  ...
  computed: {
    ...
    isOpacity(){
      return function(item){
        if(this.order && this.orderLaunch) return true
        if(item.list.length === 1) return true
        if(this.dry && item.protocol === "speed"){
            try {
              const drySpeed = this.washList[this.washModeIndex].drySpeed
              if(!drySpeed) return false
              if(drySpeed.list.length === 1) return true
            } catch (error) {
              return false
            }
        }
  ...
  watch: {
    ...
    devState: {
      handler(newV) {
        ...
        if (!this.Pow && newV !== 5) {
          console.error('整机数据异常, 此时关机整机状态却为:', newV);
          // showToastPlugin('数据异常, 请看log');
        } 
    ...
    dry: {
      ...
      handler(newV, oldV) {
        ...
        if (changeList.length < 1 && this.isActive && this.clickProtocol === 'speed') this.isActive = false; 

        let that = this;  
        this.$nextTick(() => {
          setTimeout(() => {
            this.instanceMobiScroll_speed = mobiscroll.scroller('#TimerSelector_speed', {
              theme: 'mobiscroll',
              display: 'inline',
              layout: 'liquid',
              showLabel: true,
  ...
  created() {
    if (typeof window !== 'undefined') {
      setTimeout(() => {
        document.addEventListener('touchstart', this.clickedOutside);
      }, 100);
    }
  },
  ...
  beforeDestroy() {
    if (typeof window !== 'undefined') {
      document.removeEventListener('touchstart', this.clickedOutside);
    }
  },
  ...
  methods: {
    ...
    pickerOnChange(event, inst) {
      ...
      setTimeout(() => {_this.sendCtrl(obj);},200)
      ...
      _this.devState ? sendobj.timeLeft = addTime : sendobj.timeAll = addTime;
      _this.setDataObj(sendobj);
    },
    ...
    setPickerList() {
      ...
      configList.forEach(element => {
        this[`instanceMobiScroll_${element.protocol}`] = mobiscroll.scroller(
          `#TimerSelector_${element.protocol}`,
          {
            theme: 'mobiscroll',
            display: 'inline',
            layout: 'liquid',
    ...
    handleStartBtn() {
      ...
      // 运行阶段
      if (!this.devState) {
        commitObj.runStage = runStage;
        // 第一次启动下发配置参数
        sendObj.washMode = this.washMode;
        const list = this.washList[this.washModeIndex].configList;
        list.forEach(element => {
          sendObj[element.protocol] = this[element.protocol];
        });

        const funlist = this.washList[this.washModeIndex].auxiliaryFunction;
        const setFunObj = {}
        for (const key in funlist) {
          if (funlist.hasOwnProperty(key)) {
            const value = funlist[key];
            if(key !== 'order' && key !== 'mute'){
              if(!!value && this.dataObject[key]){
                setFunObj[key] = 1
              } 
            }
          }
        }
       sendObj = {...sendObj, ...setFunObj}
      }

      this.setDataObj(Object.assign(commitObj, sendObj));
    ...
    toggle(index) {
      ...
      let length = configList[index].list.length;
      const protocolValue = configList[index].protocol;
      if (protocolValue === 'speed' && this.dry && washList.drySpeed) {
        length = washList.drySpeed.list.length;
      }
      if (length !== 1) {
