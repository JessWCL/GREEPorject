    ...
    <gree-page class="page-startup" no-navbar>
      ...
      >
        ...
        <div class="mode-text" v-else>
          {{ runStage === 13 ? '称重中' : '已完成' }}
          <p v-if="runStage === 13" class="weighing-text">
            根据衣物重量调整洗涤时间
          </p>
        </div>
        ...
        <!--洗涤--漂洗--脱水--结束 -->
        <!-- 单脱水 -->
        <gree-row v-if="washMode === 6" class="data-list-startup">
          <gree-col
            :width="dry ? 50 : 100"
        ...
        <!-- 漂洗脱水 -->
        <gree-row v-else-if="washMode === 7" class="data-list-startup">
          <gree-col
            :width="dry ? 30 : 50"
            v-for="(item, index) in startupstatus"
            :key="index"
        ...
        <!-- 其余模式 -->
        <gree-row v-else="washMode !== 6 && washMode !== 7" class="data-list-startup">
          <gree-col
            width="20"
            v-for="(item, index) in startupstatus"
            :key="index"
...
export default {
  ...
  computed: {
    ...
    startupstatus() {
      ...
      if (this.dry && list.slice(-1) !== '烘干中') list.push('烘干中')
      if (!this.dry && list.slice(-1) === '烘干中') list.remove('烘干中')
      if (this.washMode === 22 || this.washMode === 23) list = ['烘干中']
      return list
    },
    ...
    percent() {
      ...
      if (isWashADewatering) {
        ...
        addlist = [14, 74]
        if (this.dry) {
          percentRange = [2, 69, 87, 144, 180]
          percentLengthList = [2, 25, 42, 18]
          addlist = [2, 3, 45, 82]
        }
      }
      ...
      val = Math.ceil(
        (value / sum) * percentLengthList[rangeIndex - 1] +
          addlist[rangeIndex - 1],
      )
  ...
  methods: {
    ...
    goBack() {
      if (
        this.launch ||
        this.childLock ||
        (this.devState > 0 && this.devState !== 2)
      ) {
        closePagePlugin()
      } else {
        if (this.devState === 3) {
          const timeAll = this.washList[this.modeIndexList[this.washMode]]
            .defaultTime
