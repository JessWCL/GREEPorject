 ...
 * @LastEditTime: 2020-06-03 16:56:51
 -->
<template>
      ...
      <gree-row>
        <gree-col width="25" v-for="(v, k) in advancedList" :key="k">
          ...
          >
            <img
              :src="[
                require(`../assets/img/Icon/adv2/${v.selected ? 'selected' : 'normal'}/${v.IconName}.png`),
              ]"
...
import funcConfig from '../mixins/config/28a02/funcConfig';
...
export default {
  ...
  methods: {
    ...
    selectSenior(invalid, sel, row) {
      if (!invalid) {
        ...
        if (!row) {
          ...
          if (list.auxiliaryNocancel && list.auxiliaryNocancel[protocol]) {
            showToastPlugin('不可取消');
            return;
          }

          const spealList = ['nightWash', 'energySave', 'soak']; // 选择这些需要加减时间
          this.advancedList[row].selected = newV;
    ...
    handleSelect(newV, oldV, protocol) {
      ...
      const time = this.washList[this.washModeIndex].auxiliaryTimeList[protocol] >> 0;
      ...
      this.devState ? this.setDataObj({timeLeft: afterTime}) : this.setDataObj({timeAll: afterTime}); 
    },
    /**
     * @description 特殊模式需提示
     */   
