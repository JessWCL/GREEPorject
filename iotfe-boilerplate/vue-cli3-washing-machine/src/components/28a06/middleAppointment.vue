...
export default {
  ...
  computed: {
    ...mapState({
      ...
      order: state => state.parseObj.drum2Params.order,    
      orderLaunch: state => state.parseObj.drum2Params.orderLaunch,    
      orderTimeDay: state => state.parseObj.drum2Params.orderTimeDay,    
      orderTimeHour(state) {
        let value = state.parseObj.drum2Params.orderTimeHour;
        value = value > 9 ? value : `0${value}`;
        return value;
      },
      orderTimeMin(state) {
        let value = state.parseObj.drum2Params.orderTimeMin;
        value = value > 9 ? value : `0${value}`;
        return value;
      }, 
    }),
  ...
  methods: {
    ...
    handleCancelConfirm() {
      const commitObj = {};
      this.setMiddleObj(Object.assign(commitObj, { order: 0 }));
