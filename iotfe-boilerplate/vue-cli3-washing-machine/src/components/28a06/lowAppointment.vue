...
export default {
  ...
  computed: {
    ...mapState({
      ...
      orderTimeDay: state => state.parseObj.drum3Params.orderTimeDay,    
      orderTimeHour(state) {
        let value = state.parseObj.drum3Params.orderTimeHour;
        value = value > 9 ? value : `0${value}`;
        return value;
      },
      orderTimeMin(state) {
        let value = state.parseObj.drum3Params.orderTimeMin;
        value = value > 9 ? value : `0${value}`;
        return value;
      }, 
    }),
  ...
  methods: {
    ...
    handleCancelConfirm() {
      const commitObj = {};
      this.setLowObj(Object.assign(commitObj, { order: 0 }));
