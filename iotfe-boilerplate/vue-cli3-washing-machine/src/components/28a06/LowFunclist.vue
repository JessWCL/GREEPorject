...
import lowFuncConfig from '@/mixins/config/28a06/lowFuncConfig';
  ...
  methods: {
    ...
    selectSenior(item) {
      ...
      if (!item.invalid) {
        ...
        const oldV = this[protocol] >> 0;
        const newV = Boolean(value);

        const list = this.washList[this.washType][this.modeIndexList[this.washMode]];
    ...
    handleSelect(newV, oldV, protocol) {
      ...
      const time = this.washList[this.washType][this.washModeIndex].auxiliaryTime[protocol] >> 0;
      ...
      this.devState ? this.setLowObj({timeLeft: afterTime}) : this.setLowObj({timeAll: afterTime}); 
    },
