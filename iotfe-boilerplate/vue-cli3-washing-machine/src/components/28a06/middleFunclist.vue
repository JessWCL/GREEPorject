...
import middleFuncConfig from '@/mixins/config/28a06/middleFuncConfig';
import { showToastPlugin } from '@/api/pluginInterface';

export default {
  name: 'middle-funclist',
  components: {
    [Popup.name]: Popup,
    [Row.name]: Row,
    [Col.name]: Col,
  },
  mixins:[middleFuncConfig] ,
  props: {
    isPopupShow: {
      type: Object,
      default() {
  ...
  methods: {
    ...
    handleSelect(newV, oldV, protocol) {
      ...
      this.devState ? this.setMiddle({timeLeft: afterTime}) : this.setMiddle({timeAll: afterTime}); 
    },
