...
import { lowWashList, lowModeIndexList, disabledConfigObj} from '@/api/28a06/config';
...
const lowFuncConfig = {
  ...
  computed: {
    ...mapState({
      ...
      launch: state => state.parseObj.drum3Params.launch,
      timeAll: state => state.parseObj.drum3Params.timeAll,
      timeLeft: state => state.parseObj.drum3Params.timeLeft,
      childLock: state => state.parseObj.drum3Params.childLock,
      
      soak: state => state.parseObj.drum3Params.soak,
      clear: state => state.parseObj.drum3Params.clear,
      nightWash: state => state.parseObj.drum3Params.nightWash,
      creaseRes: state => state.parseObj.drum3Params.creaseRes,
      highWater: state => state.parseObj.drum3Params.highWater,

      autoMeek: state => state.parseObj.drum3Params.autoMeek,
      autoScour: state => state.parseObj.drum3Params.autoScour,
    ...
    // 高级功能列表
    advancedList() {
      const auxiliaryObj = this.washList[this.washType][this.washModeIndex].auxiliaryFunction; // 高级功能基础配置
