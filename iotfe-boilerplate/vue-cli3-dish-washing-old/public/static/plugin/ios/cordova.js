...
function PluginInterface () {
  ...
  // 获取帮助文档
  this.getHelpDocs = function(topNum,callback) {
    var arguments=[topNum];
    var callbackId=getCallBackIdWithCallBack(callback);
    var param={"arguments":arguments,"callback":callbackId};
    if(!gt_ios9()){
        navigator.gree.getHelpDocs(param);
    }
    else{
        window.webkit.messageHandlers.getHelpDocs.postMessage(param);
    }
  };

  //高泳诗+食谱的分享
  this.webShareUrlGeneral = function(url,title,brief,callback) {
    var arguments=[url,title,brief];
    var callbackId=getCallBackIdWithCallBack(callback);
    var param={"arguments":arguments,"callback":callbackId};
    if(!gt_ios9()){
        navigator.gree.webShareUrlGeneral(param);
    }
    else{
        window.webkit.messageHandlers.webShareUrlGeneral.postMessage(param);
    }
  }; 
}
