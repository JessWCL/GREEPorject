...
import { FUN_BTNS, DW_MID_828B00, WORK_STATUS } from '../api/constants';
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      functype: state => state.device.functype,
      PurifierTime: state => state.device.DataObject.PurifierTime,
      PauseStart: state => state.device.DataObject.PauseStart,
      TmrHour: state => state.device.DataObject.TmrHour,
      TmrMin: state => state.device.DataObject.TmrMin,
      RmTmrHour: state => state.device.DataObject.RmTmrHour,
      RmTmrMin: state => state.device.DataObject.RmTmrMin,
      WorkStatus: state => state.device.DataObject.WorkStatus,
      StTmr: state => state.device.DataObject.StTmr,
      Wash: state => state.device.DataObject.Wash,
      WashDone: state => state.device.DataObject.WashDone,
      Dry: state => state.device.DataObject.Dry,
      TmrOn: state => state.device.DataObject.TmrOn,
      funBtnList: state => state.device.funBtnList,

      computedTmrHour: state => {
        const TmrHour = state.device.DataObject.TmrHour;
        const TmrMin = state.device.DataObject.TmrMin;
      ...
      isBlueColor(state) {
        const modeBtnList = state.device.modeBtnList;
        let ret = true;
  ...
  methods: {
    ...mapActions('device', ['handleRenderCanvas']),
    ...mapActions('device', ['handleClickStart']),
    ...mapActions('device', ['handleClickCancle']),
    ...mapActions('device', ['handleClickPause']),
    ...mapActions('device', ['handleClickConfirm']),
    ...mapActions('device', ['handleDashBoardAppointmentCancel']),
    ...mapActions('device', ['handleClickPurifierBtn']),
