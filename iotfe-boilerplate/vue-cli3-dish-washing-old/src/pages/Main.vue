...
import { BarColor } from '../api/constants';
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      modeBtnList: state => state.device.modeBtnList,
      isPurifierAppointment: state => state.device.isPurifierAppointment,
      workStatusName: state => state.device.workStatusName,
      workStatusImgSrc: state => state.device.workStatusImgSrc,
      workStatusImgBG: state => state.device.workStatusImgBG,
      carouselId: state => state.device.carouselId,
      isAppointment: state => state.device.isAppointment,
      ...
      carouselOptions(state) {
        const { Wash, TmrOn } = state.device.DataObject;
      ...
      isCarouselLocking: function isCarouselLocking(state) {
        if (state.device.DataObject.Wash || state.device.DataObject.TmrOn) {
  ...
  methods: {
    ...mapActions('device', ['changeModeFromCarouselId']),
    ...mapMutations('device', ['setIsPurifierAppointment']),
