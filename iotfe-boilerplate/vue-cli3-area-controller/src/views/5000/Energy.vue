<template>
  <gree-view bg-color="#F4F4F4">
    ...
    <div class="page-content-se">
      <gree-list>
        <gree-list-item :title="$language('btn.advance_SE')">
          <gree-switch 
            slot="after" 
...
export default {
  ...
  data() {
    return {
      devname: this.$language('btn.advance_SE'),
      isActive2: false,
      pickerData: Data1,
  ...
  mounted() {
    this.isActive2 = Boolean(this.EnSvSt);
    const pickerIndex =
      this.Mod === 1 ? this.CoolSvStTemMin + 16 : this.HeatSvStTemMax + 16;
  ...
  methods: {
    ...
    svstSave() {
      ...
      if (this.Mod === 1) {
        this.sendCtrl({
          CoolSvStTemMin: Number(this.pickerValue - 16),
          EnSvSt: SvstValue
        });
        this.setDataObject({
          CoolSvStTemMin: this.pickerValue - 16,
          EnSvSt: SvstValue
        });
      } else {
        this.sendCtrl({
          HeatSvStTemMax: Number(this.pickerValue - 16),
          EnSvSt: SvstValue
        });
        this.setDataObject({
          HeatSvStTemMax: this.pickerValue - 16,
          EnSvSt: SvstValue
        });
      }
      this.$router.go(-1);
