  ...
  <gree-view :bg-color="statusBarColor">
    <gree-page no-navbar class="page-home">
      <div class="page-header" :style="{ backgroundImage: 'url(' + head_bg + ')' }">
        ...
        />
        <div class="battery-remind" v-if="gas_sensor_state > 0">
          <img :src="alarmImg" />
        </div>
      ...
      <div v-show="false">{{ gas_sensor_state }} - {{ ColorChange }}</div>
    </gree-page>
  </gree-view>
</template>
  ...
  mounted() {
    ...
    tuyaGetDevLogs(this.mac, startTime, endTime, logType, logSize)
      .then(res => {
        ...
        if (__res.code === 200) {
          this.lists = __res.message.result.logs.filter(item => item.code === 'gas_sensor_state');
