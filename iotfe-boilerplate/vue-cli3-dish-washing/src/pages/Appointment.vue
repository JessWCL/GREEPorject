...
import { FUN_BTNS, DW_MID_828B00, MOD_NONE } from '../api/constant';
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      upperIsForbided: state => state.funBtnList[UP_WASH].isForbided, // 按钮是否禁用
      underIsForbided: state => state.funBtnList[DOWN_WASH].isForbided, // 按钮是否禁用
      dryIsForbided: state => state.funBtnList[HOT_AIR_DRYING].isForbided, // 按钮是否禁用
      purifierIsForbided: state => state.funBtnList[PURIFIER].isForbided, // 按钮是否禁用
      TmrStatus: state => state.DataObject.TmrStatus,
  ...
  watch: {
    TmrStatus(newV, oldV) { // 监听预约状态，不同的手机，预约后界面不同步的问题
      if (newV === 1 && oldV === 0) {
        vm.$router.push('/Home'); // eslint-disable-line
      }
    },
  ...
  methods: {
    ...mapActions(['handleSetStTmr']),
    ...mapActions(['handleAppointmentCancel']),
    ...mapActions(['handleAppointmentStart']),
    ...mapActions(['handleAppointmentLayerWash']),
    ...mapActions(['handleAppointmentSetDry']),
    ...mapActions(['handleAppointmentSetPurifier']),
    ...mapMutations(['setTmrHour']),
    ...mapMutations(['setTmrMin']),
